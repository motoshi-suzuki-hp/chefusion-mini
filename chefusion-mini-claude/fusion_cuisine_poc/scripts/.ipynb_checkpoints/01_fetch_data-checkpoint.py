...
def create_dummy_images(recipes_df: pd.DataFrame, images_dir: Path) -> None:
    ...
    # Define color palettes for different cuisines
    color_palettes = {
        "japanese": [(139, 69, 19), (205, 133, 63), (160, 82, 45), (210, 180, 140)],  # Brown tones
        "italian": [(220, 20, 60), (178, 34, 34), (255, 99, 71), (255, 140, 0)]  # Red/orange tones
    }
...
def create_dummy_image(size: int, base_color: Tuple[int, int, int]) -> Image.Image:
    ...
    # Add some random shapes with slightly different colors
    for _ in range(random.randint(3, 8)):
        # Create color variations (±30 from base color)
        color_var = tuple(
            max(0, min(255, base_color[i] + random.randint(-30, 30))) 
            for i in range(3)
        )
        ...
        if random.choice([True, False]):
            ...
            x2, y2 = x1 + random.randint(20, size//3), y1 + random.randint(20, size//3)
